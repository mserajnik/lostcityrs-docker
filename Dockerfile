...
RUN \
  # `openjdk-21-jdk` is required only by versions `244` and `245.2`, whose
  # startup cache packing compiles RuneScript with a Java tool; the other
  # versions this image builds (`225` and `254`) do not use it but share this
  # Dockerfile. It also transitively provides `ca-certificates`, which the
  # HTTPS `fixuid` download below relies on; if Java is ever removed entirely,
  # `ca-certificates` has to be added to the install list explicitly.
  apt update && \
